...
### 标准数独传播（Sudoku Constraint Propagation）
...
### 笼和极值（Minimum and Maximum Cage Sums）
...
### 固定组合与组合表（Fixed Combinations and Combination Tables）
...
### 笼组合过滤（Cage Combination Filtering）
...
### 笼内排列与位置分析（Cage Placement Analysis）
...
### 多 house 的 45 法则（Multiple-House 45 Rule）
...
### 笼拆分与虚拟笼（Cage Splitting and Pseudo-Cages）
...
### 最小剩余值搜索（Minimum Remaining Values Search）
