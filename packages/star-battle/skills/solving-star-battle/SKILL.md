...
description: Use when a Star Battle puzzle's regions matrix and k value are already decoded and confirmed by the user (typically via decoding-star-battle), and the puzzle now needs to be solved with step-by-step reasoning. Writes output.json (regions+k+solution+steps), then invokes rendering-star-battle to display the solved board.
---
...
digraph flow {
    ...
    "invoke rendering-star-battle /tmp/sb-output.json" [shape=box];
    "拿到已确认 input.json" -> "调 solve-board → 写 /tmp/sb-output.json"
        -> "invoke rendering-star-battle /tmp/sb-output.json";
}
```

**前置**：本 skill 假定 input.json 的 `regions` 和 `k` **已经被用户看过并确认**。如果调用方是 [[decoding-star-battle]]，那 skill 在交棒前已完成 invoke rendering 让用户确认。
...
node_modules/.bin/tsx packages/star-battle/skills/solving-star-battle/references/solve-board.ts /tmp/sb-input.json /tmp/sb-output.json

# 在已安装 plugin 中：
node_modules/.bin/tsx skills/solving-star-battle/references/solve-board.ts /tmp/sb-input.json /tmp/sb-output.json
```
...
不要直跑 render-board，**invoke** [[rendering-star-battle]]，把 output.json 路径传过去。rendering 读 `regions + k + solution` 即可画带 ★ 的盘。
...
| 自己跑 render-board 显示解 | **不可**。invoke rendering-star-battle，把 output.json 路径传给它。 |
