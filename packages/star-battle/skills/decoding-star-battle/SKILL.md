...
把用户给的 Star Battle 谜题图片转成 `{regions, k}` JSON，**渲染给用户确认**后交棒给 [[solving-star-battle]] 求解。求解本身不属于本 skill。
...
- **regions**：给每个单元格分配区域 id（从 0 开始的整数）。区域分割看**粗线**或**底色**：
  ...
  - 颜色版：同色 = 同区
  - 复杂图也可能两者并用
- **k**：题目通常会写"X stars per row, column and region"（中文"每行/列/区 X 颗星"）。**没有默认值** — 图中没标且未确认时，必须用 AskUserQuestion 询问用户。
...
用户确认后，**必须 invoke** [[solving-star-battle]] 求解，不要自己跑 `solve-board.ts` 或心算给答案。solving skill 会再次渲染（带星位）展示最终解。
...
| 自己跑 solve-board.ts | 求解归 solving-star-battle，invoke 它。 |
...
- "我直接读 input.json 心算给答案" → 不可。invoke solving-star-battle
